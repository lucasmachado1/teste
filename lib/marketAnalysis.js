...
function buildRiskPlan({ candles, currentPrice, signal, forecast, volatility20 }) {
  if (!currentPrice || !Number.isFinite(currentPrice)) return null;

  const direction = signal.score >= 1 ? 'LONG' : signal.score <= -1 ? 'SHORT' : 'NEUTRO';
  const precision = currentPrice > 100 ? 2 : 5;
  const atr14 = averageTrueRange(candles, 14);
  const volatilityDistance = volatility20 ? currentPrice * (volatility20 / 100) : 0;
  const minimumDistance = currentPrice * 0.006;
  const riskDistance = Math.max(atr14 ? atr14 * 1.15 : 0, volatilityDistance * 1.1, minimumDistance);
  const basePlan = {
    direction,
    entry: round(currentPrice, precision),
    atr14: round(atr14, precision),
    riskPercent: round((riskDistance / currentPrice) * 100, 2),
    timeFrame: '5m',
    leverage: 'spot/baixo risco',
    notes: 'Use tamanho de posição compatível com seu capital; evite operar se o spread ou a liquidez estiverem ruins.'
  };
  ...
  if (direction === 'NEUTRO') {
    return {
      ...
      takeProfits: [],
      rewardToRisk: null,
      invalidation: 'Sem entrada sugerida enquanto o score técnico permanecer neutro.'
    };
  }

  const multiplier = direction === 'LONG' ? 1 : -1;
  const stopLoss = currentPrice - (riskDistance * multiplier);
  const takeProfit1 = currentPrice + (riskDistance * 1.2 * multiplier);
  const takeProfit2 = currentPrice + (riskDistance * 2 * multiplier);
  const takeProfit3 = currentPrice + (riskDistance * 3 * multiplier);
  const projectedPrice = forecast && forecast.projectedPrice ? forecast.projectedPrice : null;
  const projectedSupportsDirection = projectedPrice
    ? (direction === 'LONG' ? projectedPrice > currentPrice : projectedPrice < currentPrice)
    : false;

  return {
    ...basePlan,
    action: direction === 'LONG' ? 'comprar em pullback/rompimento' : 'vender/short em pullback/rompimento',
    stopLoss: round(stopLoss, precision),
    takeProfits: [takeProfit1, takeProfit2, takeProfit3].map((target, index) => ({
      label: `TP${index + 1}`,
      price: round(target, precision),
      gainPercent: round(Math.abs(percent(target, currentPrice)), 2)
    })),
    rewardToRisk: '1.2 / 2.0 / 3.0',
    invalidation: direction === 'LONG'
      ? 'Perde força se fechar candle de 5m abaixo do stop ou se EMA12 cruzar abaixo da EMA26.'
      : 'Perde força se fechar candle de 5m acima do stop ou se EMA12 cruzar acima da EMA26.',
    notes: projectedSupportsDirection
      ? `${basePlan.notes} A previsão estatística confirma a direção do setup.`
      : `${basePlan.notes} A previsão estatística não confirma totalmente a direção; reduza risco ou aguarde confirmação.`
  };
}
...
async function fetchBinanceMarketData(asset, interval = '5m', limit = 120) {
...
async function fetchCoinbaseMarketData(asset, limit = 120) {
  const candleUrl = `${COINBASE_BASE_URL}/products/${asset.coinbaseProduct}/candles?granularity=300`;
  ...
  const candles = rawCandles
    ...
    .map((candle) => ({
      ...
      close: Number(candle[4]),
      volume: Number(candle[5]),
      closeTime: candle[0] * 1000 + 299999
    }))
    .reverse();
...
async function analyzeAsset(asset) {
  ...
  const volatility20 = volatility(closes, 20);
  const forecast = forecastPrice(closes, currentPrice, sma20, sma50, ema12, ema26, rsi14);
  ...
  return {
    ...
    indicators: {
      ...
      rsi14: round(rsi14, 2),
      atr14: riskPlan ? riskPlan.atr14 : null,
      volatility20: round(volatility20, 3)
    },
    signal,
    forecast,
    riskPlan,
...
function formatTelegramMessage(report) {
  ...
  report.assets.forEach((asset) => {
    ...
    lines.push(`Previsão ${asset.forecast.horizon}: $${asset.forecast.projectedPrice} (${asset.forecast.projectedChange}%)`);
    if (asset.riskPlan && asset.riskPlan.direction !== 'NEUTRO') {
      const targets = asset.riskPlan.takeProfits.map((target) => `${target.label} $${target.price}`).join(' | ');
      lines.push(`Plano: *${asset.riskPlan.direction}* | Entrada $${asset.riskPlan.entry}`);
      lines.push(`Take profits: ${targets}`);
      lines.push(`Stop loss: $${asset.riskPlan.stopLoss} | R:R ${asset.riskPlan.rewardToRisk}`);
    } else {
      lines.push('Plano: *AGUARDAR* | Sem entrada com score neutro.');
    }
